...
using RenderOptions = Carto.MapModel.RenderOptions;

namespace Carto.Cartographer
{
    class VisualBitmapCache: FrameworkElement
    {
        class CachedMapArea
        {
        ...
        private int ENTIRE_MAP_PIXELS = 5000000;
        ...
        public VisualBitmapCache()
        {
            // Setup the timer so it pauses REDRAW_INTERVAL ms before refreshing the detail map.
            timer = new DispatcherTimer(new TimeSpan(0, 0, 0, 0, REDRAW_INTERVAL), DispatcherPriority.Background, new EventHandler(CacheDetailMap), Dispatcher);
            timer.Stop();
        }
